...
namespace Downlink
{
    public class HSLColor
    {
        ...
        /// index = [0,range]
        public static Color FetchCachedHue(int index, int range)
        {
            var hue = (int)((240f * index) / range);
            var w = ColorCache[hue];
            if (w != null) return w.Color;
            var c = (Color)new HSLColor(hue, 240.0d, 120.0d);
            ColorCache[hue] = new ColorWrapper(c);
            return c;
        }
    ...
    {
        public ColorWrapper(Color c) { this.Color = c; }
